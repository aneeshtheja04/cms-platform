...
export default function LessonAssetsManager({ lesson, onUpdate }) {
    const { hasRole } = useAuth();
    const canEdit = hasRole(['admin', 'editor']);
    const [assets, setAssets] = useState(lesson.assets || []);
    const [loading, setLoading] = useState(false);
    const [newAssetUrl, setNewAssetUrl] = useState('');
    const [addingTo, setAddingTo] = useState(null); // { language, variant }

    useEffect(() => {
        setAssets(lesson.assets || []);
    }, [lesson]);

    const variants = ['portrait', 'landscape'];

    const getAsset = (lang, variant) => {
        return assets.find(
            (a) => a.language === lang && a.variant === variant && a.asset_type === 'thumbnail'
        );
    };
    ...
    const handleAddAsset = async (lang, variant) => {
        ...
        try {
            ...
            const newAsset = await assetsApi.createLessonAsset(data);
    ...
    const handleDeleteAsset = async (id) => {
        ...
        try {
            setLoading(true);
            await assetsApi.deleteLessonAsset(id);
    ...
    // Only show assets for languages available in the lesson content
    const contentLanguages = lesson.content_languages_available || [lesson.content_language_primary];
        ...
        <div className="space-y-8">
                ...
                <div key={lang} className="border border-gray-200 rounded-lg p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {variants.map((variant) => {
                            ...
                            const isRequired = lang === lesson.content_language_primary;
            ...
            {contentLanguages.length === 0 && (
                <div className="text-center text-gray-500 italic">
                    No content languages configured. Please add content languages in the settings tab first.
                </div>
            )}
        </div>
    );
